...
collection = client['meteo']['metar']

all_data = list(collection.find({'stasiun' : 'WAAA'}))
timeseries_wind = []
timeseries_only = []
for i in range(len(all_data)):
	kec_angin = all_data[i]['kecepatan_angin']
...
#	print('%s%s%s%s%s'%(tahun, bulan, hari, jam, menit), kec_angin)
	timeseries_wind.append(['%s%s%s%s%s'%(tahun, bulan, hari, jam, menit), kec_angin])
...
while year_current < end_year:
	...
	try:
		...
		real_timeseries_wind.append([waktu_gen_str, 'NO DATA'])
